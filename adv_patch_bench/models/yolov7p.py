...
from yolov7.utils.boxes import bboxes_iou, postprocess

from adv_patch_bench.models.custom_build import CUSTOM_META_ARCH_REGISTRY

__all__ = ["YOLOV7P", "YOLOHead"]
    ...
    def __init__(self, cfg):
        ...
        if self.neck_type == "fpn":
            ...
            self.m = nn.ModuleList(
                nn.Conv2d(
                    x,
                    len(cfg.MODEL.YOLO.ANCHORS[0])
                    * (5 + cfg.MODEL.YOLO.CLASSES),
                    1,
                )
        ...
        self.loss_evaluators = [
            YOLOHead(cfg, anchor, level, self.device)
...
class YOLOHead(nn.Module):
    def __init__(self, cfg, anchors, level, device):
        super().__init__()
        self.level = level
        self.loss_type = cfg.MODEL.YOLO.LOSS_TYPE
