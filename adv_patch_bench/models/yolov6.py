...
Code adapted from
https://github.com/jinfagang/yolov7_d2/blob/main/yolov7/modeling/meta_arch/yolov6.py
"""
...
class YOLOV6(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        logger.info(
            "backboneshape: %s, size_divisibility: %s",
            str(backbone_shape),
            str(self.size_divisibility),
        )
        ...
        # TODO(Enhancement): Normalize should be module buffers
        pixel_mean = (
    ...
    def preprocess_image(self, batched_inputs, compute_loss):
        """Preprocess image."""
        ...
        if (
            compute_loss
            and self.iter > self.enable_l1_loss_at
            and not self.use_l1
        ):
            ...
            comm.synchronize()
            logger.info("check head l1: %s", str(self.head.use_l1))
    ...
    def preprocess_input(self, images: torch.Tensor) -> torch.Tensor:
        """Preprocess input."""
    ...
    def forward(
        self, batched_inputs: torch.Tensor, compute_loss: bool = False
    ) -> torch.Tensor:
        """Forward function."""
        if self.onnx_export:
            logger.info("[WARN] exporting onnx...")
            assert isinstance(
                batched_inputs, (torch.Tensor, list)
