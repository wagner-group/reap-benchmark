...
from yolof.modeling.box_regression import YOLOFBox2BoxTransform
from yolof.modeling.decoder import Decoder
from yolof.modeling.encoder import DilatedEncoder
from yolof.modeling.uniform_matcher import UniformMatcher
...
class YOLOF(nn.Module):
    ...
    def losses(
        ...
        pred_class_logits = cat(pred_class_logits, dim=1).view(
            -1, self.num_classes
        )
        ...
        for i in range(N):
            ...
            iou = box_iou(
                predicted_boxes[i, ...], gt_instances[i].gt_boxes.tensor
            )
            if iou.numel() == 0:
        ...
        # EDIT: src_idx ends up on CPU which causes error when indexed by
        # pos_ignore_idx
        src_idx = src_idx.to(pred_class_logits.device)
