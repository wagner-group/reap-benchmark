...
class CustomCOCOEvaluator(DatasetEvaluator):
    ...
    def evaluate(self, img_ids: Optional[List[int]] = None) -> Dict[str, Any]:
        ...
        if "instances" in predictions[0]:
            self._eval_predictions(
                set(self._tasks), predictions, img_ids=img_ids
            )
    ...
    def _derive_coco_results(self, coco_eval, iou_type, class_names=None):
        ...
        results = {
            metric: float(
                coco_eval.stats[idx] * 100
                if coco_eval.stats[idx] >= 0
                else "nan"
            )
        ...
        for idx, name in enumerate(class_names):
            ...
            ap_50_95.append(ap if not np.isnan(ap) else 0)
...
def _evaluate_predictions_on_coco(
    ...
    if iou_type == "keypoints":
        ...
        num_keypoints_gt = (
            len(next(iter(coco_gt.anns.values()))["keypoints"]) // 3
        )
