...
def main(args):
    ...
    final_df = final_df[
        (final_df["final_shape"] != "circle-750.0")
        & (final_df["points"].isna())
        & (final_df["final_shape"] != "other-0.0-0.0")
        & (final_df["final_shape"] == "octagon-915.0")
    ]
    ...
    corrections_df.to_csv(
        f"use_polygons_corrections_{split}_df.csv", index=False
    )

    main_df = pd.read_csv(f"mapillary_vistas_{split}_final_merged.csv")
    main_df = main_df.merge(
        corrections_df,
        on=["filename", "object_id"],
        how="left",
        suffixes=("", "_polygon"),
    )
    print("saving df")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        "Dataset Preperation", parents=[get_args_parser()]
    )
