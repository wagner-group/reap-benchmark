...
def crop_traffic_signs(
    filename, panoptic_per_image_id, img_path, label_path, min_area=0, pad=0.1
):
    ...
    img_padded, pad_size = pad_image(
        img, pad_mode="constant", return_pad_size=True, pad_size=0.25
    )
    ...
    for obj in segment:
        ...
        is_oob = (
            (xmin == 0)
            or (ymin == 0)
            or ((xmin + width) >= img_width - 1)
            or ((ymin + height) >= img_height - 1)
        )
        ...
        ymin, xmin = max(0, ymin + pad_size - extra_pad), max(
            0, xmin + pad_size - extra_pad
        )
        temp_mask = (
            id_padded[
                ymin : ymin + height + 2 * extra_pad,
                xmin : xmin + width + 2 * extra_pad,
            ]
            == obj["id"]
        ).astype(np.uint8)
        ...
        bool_mask = (id_padded[ymin:ymax, xmin:xmax] == obj["id"]).astype(
            np.uint8
        )
...
def main():
    ...
    for filename in tqdm(filenames):
        ...
        offset_df = save_offset(
            output, filename.split(".")[0], save_paths, offset_df
        )
    offset_df.to_csv(f"offset_{SPLIT}.csv", index=False)


def save_images(output, filename, paths):
    for img, mask, obj_id in zip(
        output["images"], output["masks"], output["obj_id"]
    ):
        Image.fromarray(img, "RGB").save(
            join(paths[0], f"{filename}_{obj_id}.png")
        )
        Image.fromarray(mask * 255).save(
            join(paths[1], f"{filename}_{obj_id}.png")
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Dataset Preperation", add_help=False
    )
