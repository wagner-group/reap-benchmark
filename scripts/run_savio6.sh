...
#SBATCH --output slurm-%j-synthetic-10x20-obj64-norelight-tlsc-pd64-ld0.00001.out  # TODO
## Command(s) to run:
...
MASK_SIZE=10x20
SYN_OBJ_SIZE=64
ATK_CONFIG_PATH=./configs/attack_config_savio6.yaml
...
function syn_attack {
    ...
    python -u test_detectron.py \
        ...
        --mask-name "$MASK_SIZE" --weights $MODEL_PATH --workers $NUM_WORKERS \
        --transform-mode $TF_MODE --img-txt-path $BG_FILES --attack-type load \
        --annotated-signs-only --no-patch-relight &&
